'use client';
import {
  useScroll,
  useTransform,
  motion,
  useInView,
} from 'framer-motion';
import React, { useLayoutEffect, useRef, useState } from 'react';

interface TimelineEntry {
  title: string;
  content: React.ReactNode;
}

// ───────────────────────────────────────────────────────────────────────────────
// Item individual del Timeline
// ───────────────────────────────────────────────────────────────────────────────
const TimelineItem = ({ item }: { item: TimelineEntry; index: number }) => {
  const itemRef = useRef<HTMLDivElement>(null);
  const isInView = useInView(itemRef, {
    once: true,
    margin: '-20% 0px -20% 0px',
  });

  // Variants
  const containerVariants = {
    hidden: { opacity: 1 },
    visible: {
      opacity: 1,
      transition: {
        staggerChildren: 0.03,
        delayChildren: 0.15,
      },
    },
  };

  const dotVariants = {
    hidden: { opacity: 0, scale: 0 },
    visible: {
      opacity: 1,
      scale: 1,
      transition: {
        duration: 0.6,
        ease: [0.68, -0.55, 0.265, 1.55], // back.out(1.7)
      },
    },
  };

  const yearVariants = {
    hidden: { opacity: 0, x: -100 },
    visible: {
      opacity: 1,
      x: 0,
      transition: { duration: 0.4, ease: 'easeOut' },
    },
  };

  const contentVariants = {
    hidden: { opacity: 0, x: 100 },
    visible: {
      opacity: 1,
      x: 0,
      transition: { duration: 0.5, ease: 'easeOut' },
    },
  };

  return (
    <motion.div
      ref={itemRef}
      className="flex justify-start pt-10 md:gap-10 md:pt-40"
      initial="hidden"
      animate={isInView ? 'visible' : 'hidden'}
      variants={containerVariants}
    >
      <div className="sticky top-40 z-40 flex max-w-xs flex-col items-center self-start md:w-full md:flex-row lg:max-w-sm">
        {/* DOT: sólido (rojo), sin gradientes raros */}
        <motion.div
          className="absolute left-3 flex h-10 w-10 items-center justify-center rounded-full border border-gray-700 bg-gray-900 md:left-3"
          variants={dotVariants}
        >
          <div className="h-4 w-4 rounded-full border border-red-400 bg-red-500" />
        </motion.div>

        {/* Año (rojo) */}
        <motion.h3
          className="hidden text-xl font-bold text-red-400 md:block md:pl-20 md:text-5xl"
          variants={yearVariants}
        >
          {item.title}
        </motion.h3>
      </div>

      <div className="relative w-full pl-20 pr-4 md:pl-4">
        {/* Año (mobile) */}
        <motion.h3
          className="mb-4 block text-left text-2xl font-bold text-red-400 md:hidden"
          variants={yearVariants}
        >
          {item.title}
        </motion.h3>

        <motion.div variants={contentVariants}>{item.content}</motion.div>
      </div>
    </motion.div>
  );
};

// ───────────────────────────────────────────────────────────────────────────────
// Timeline principal
// ───────────────────────────────────────────────────────────────────────────────
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  const ref = useRef<HTMLDivElement>(null);
  const containerRef = useRef<HTMLDivElement>(null);
  const [height, setHeight] = useState(0);

  // Medición robusta de altura (y en resize)
  useLayoutEffect(() => {
    function measure() {
      if (ref.current) {
        const rect = ref.current.getBoundingClientRect();
        setHeight(rect.height);
      }
    }
    measure();
    window.addEventListener('resize', measure);
    return () => window.removeEventListener('resize', measure);
  }, []);

  const { scrollYProgress } = useScroll({
    target: containerRef,
    offset: ['start 10%', 'end 90%'], // recorre más del contenedor para notar el avance
  });

  const heightTransform = useTransform(scrollYProgress, [0, 1], [0, height]);
  const opacityTransform = useTransform(scrollYProgress, [0, 0.1], [0, 1]);

  return (
    <div className="w-full bg-transparent font-sans md:px-10" ref={containerRef}>
      <div className="mx-auto max-w-7xl px-4 md:px-8 lg:px-10" />

      <div ref={ref} className="relative mx-auto max-w-7xl pb-20">
        {data.map((item, index) => (
          <TimelineItem key={index} item={item} index={index} />
        ))}

        {/* Track tenue */}
        <div
          style={{
            height: `${height}px`,
            backgroundColor: 'rgba(55,65,81,0.35)', // gris 700/35%
            WebkitMaskImage:
              'linear-gradient(to bottom, transparent 0%, black 10%, black 90%, transparent 100%)',
            maskImage:
              'linear-gradient(to bottom, transparent 0%, black 10%, black 90%, transparent 100%)',
          }}
          className="absolute left-8 top-0 w-[2px] overflow-hidden pointer-events-none z-10 md:left-8 rounded"
        >
          {/* Progreso con el gradiente pedido (rojo→negro con highlight cálido) */}
          <motion.div
            style={{
              height: heightTransform,
              opacity: opacityTransform,
              background: `
                linear-gradient(
                  to top,
                  rgba(127,29,29,0.60) 0%,    /* #7f1d1d99 aprox */
                  rgba(185,28,28,0.85) 16%,   /* #b91c1cdd */
                  #ffffff 38%,                /* highlight central */
                  rgba(239,68,68,0.80) 62%,   /* #ef4444cc */
                  rgba(0,0,0,0.00) 100%
                )
              `,
              boxShadow: '0 0 16px rgba(239,68,68,0.35)',
            }}
            className="absolute inset-x-0 top-0 w-[2px] rounded-full z-20"
          />
        </div>
      </div>
    </div>
  );
};
